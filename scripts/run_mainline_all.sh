...
# For each of the 4 backbones x per-setting k-grid x seeds, runs 3 methods:
#   safepatch (ours, no-gate cr10) | distrep (rival) | arachne (rival)
...
METHODS="${METHODS:-safepatch distrep arachne}"
...
for bb in resnet50 convnext_tiny densenet121 vgg16; do
  ...
  for m in $METHODS; do
    case "$m" in
      ...
      distrep)   bash scripts/run_fewshot_distrep.sh   "$setting" "$SEEDS" "$ks" || echo "[ERR distrep] $setting";;
      arachne)   bash scripts/run_fewshot_arachne.sh   "$setting" "$SEEDS" "$ks" || echo "[ERR arachne] $setting";;
    esac
  done
done
