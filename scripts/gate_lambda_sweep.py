#!/usr/bin/env python3
"""Cost-weighted gate threshold, as an alternative to the plain theta=0 rule.
...
a-priori grid of lambda values. Unlike a quantile-q sweep, lambda touches the report
...
DynaPatch's own protocol-C fit only (gate_protocol_b.py's "both"/"calib" default) -- an earlier
version also computed the same sweep for the gate transplanted onto NN-Patching/PatchNAS's raw
patches (analysis_gate_transplant.py's "dgen" protocol), but the paper's RQ4 table is DynaPatch-
only, so that half was dropped (2026-09-21) rather than computed and left unread; see git history
if that comparison is needed again.
...
def dp_curve() -> pd.DataFrame:
    ...
    for k in sorted(set(train) & set(report)):
        ...
        # disagreed with `outputs/rq4/gate_natural_point_protocolC_ep40ns.csv` (names.py's ONLY
...
def main() -> None:
    ...
    out = ROOT / "outputs" / "rq4" / "gate_lambda_sweep"
...
sweep (`outputs/rq4/gate_curves_protocolC_ep40ns.csv`,
`outputs/rq3/gate_transplant/curve_4pre+post.csv`). lambda in {{{', '.join(str(l) for l in LAMBDAS)}}}
...
""",
    ...
    print(f"[written] outputs/rq4/gate_lambda_sweep/per_cell.csv  ({len(cell)} rows)")
