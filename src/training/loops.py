...
from src.evaluation.metrics import compute_binary_regression_rate
...
def train_repair_with_dual_objectives(
    model: nn.Module,
    base_model: nn.Module,
    bug_loader: DataLoader,
    clean_loader: DataLoader,
    optimizer: torch.optim.Optimizer,
    repair_loss_fn: nn.Module,
    robust_loss_fn: nn.Module | None,
    cert_loss_fn: nn.Module | None,
    field_loss_fn: nn.Module | None,
    decision_ball_loss_fn: nn.Module | None,
    lambda_repair: float,
    lambda_robust: float,
    lambda_cert: float,
    lambda_field: float,
    lambda_decision_ball: float,
    lambda_patch_l2: float,
    lambda_clean_replay: float,
    cert_on_bug: bool,
    cert_every_n_steps: int,
    cert_clean_batch_limit: int | None,
    ...
    """Train with the paper-aligned shift and guard objectives."""
    _freeze_backbone_modules(model)
    model.train()
    base_model.eval()
    ...
    for batch_index, batch in enumerate(bug_loader):
        ...
        repair_loss = repair_loss_fn(logits, labels)
        robust_loss = torch.zeros((), device=device)
        cert_loss = torch.zeros((), device=device)
        field_loss = torch.zeros((), device=device)
        decision_ball_loss = torch.zeros((), device=device)
        patch_reg_loss = torch.zeros((), device=device)
        clean_replay_loss = torch.zeros((), device=device)
        clean_stats = {"clean_route_hit": 0.0, "cert_margin": 0.0}

        if robust_loss_fn is not None and lambda_robust > 0.0:
            robust_loss, _ = robust_loss_fn(model=model, inputs=inputs, labels=labels)
        cert_active_this_step = (
            cert_loss_fn is not None
            and lambda_cert > 0.0
            and (batch_index % cert_every_n_steps == 0)
        ...
        if cert_active_this_step:
            if cert_on_bug:
                ...
                cert_labels = clean_labels
            cert_loss, clean_stats = cert_loss_fn(model=model, inputs=cert_inputs, labels=cert_labels)
        if field_loss_fn is not None and lambda_field > 0.0:
            field_loss, _field_stats = field_loss_fn(model=model, inputs=inputs, labels=labels)
        if decision_ball_loss_fn is not None and lambda_decision_ball > 0.0:
            decision_ball_loss, _decision_stats = decision_ball_loss_fn(model=model, inputs=inputs, labels=labels)
        if lambda_patch_l2 > 0.0:
            patch_reg_loss = outputs["gated_patch"].flatten(1).norm(dim=1).mean()  # [batch]
        if lambda_clean_replay > 0.0:
            clean_inputs, clean_labels = _move_batch(next(clean_iter), device)
        ...
        if loss_normalization == "detach_ratio":
            repair_term = repair_loss / repair_loss.detach().clamp_min(1.0e-8)
            robust_term = robust_loss / robust_loss.detach().clamp_min(1.0e-8)
            cert_term = cert_loss / cert_loss.detach().clamp_min(1.0e-8)
            field_term = field_loss / field_loss.detach().clamp_min(1.0e-8)
            decision_ball_term = decision_ball_loss / decision_ball_loss.detach().clamp_min(1.0e-8)
            patch_reg_term = patch_reg_loss / patch_reg_loss.detach().clamp_min(1.0e-8)
            clean_replay_term = clean_replay_loss / clean_replay_loss.detach().clamp_min(1.0e-8)
        elif loss_normalization == "none":
            repair_term = repair_loss
            robust_term = robust_loss
            cert_term = cert_loss
            field_term = field_loss
            decision_ball_term = decision_ball_loss
            patch_reg_term = patch_reg_loss
            clean_replay_term = clean_replay_loss
        else:
            raise ValueError(f"Unsupported loss normalization mode: {loss_normalization}")

        total_objective = (
            float(lambda_repair) * repair_term
            + lambda_robust * robust_term
            + lambda_cert * cert_term
            + lambda_field * field_term
            + lambda_decision_ball * decision_ball_term
            + float(lambda_patch_l2) * patch_reg_term
            + float(lambda_clean_replay) * clean_replay_term
        )
        total_objective.backward()
        if grad_clip_norm is not None:
            torch.nn.utils.clip_grad_norm_(model.parameters(), grad_clip_norm)
        ...
        total_correct += float((predictions == labels).float().sum().item())
        total_samples += batch_size
        robust_total += float(robust_loss.item())
        cert_total += float(cert_loss.item())
        field_total += float(field_loss.item())
        decision_ball_total += float(decision_ball_loss.item())
        patch_reg_total += float(patch_reg_loss.item())
        clean_replay_total += float(clean_replay_loss.item())
        clean_route_hit_total += float(clean_stats["clean_route_hit"])
        cert_margin_total += float(clean_stats["cert_margin"])
        ...
        if log_every_n_batches is not None and log_every_n_batches > 0 and ((batch_index + 1) % log_every_n_batches == 0):
            print(
                f"[TrainStep] batch={batch_index + 1} "
                f"repair={float(repair_loss.item()):.4f} "
                f"robust={float(robust_loss.item()):.4f} "
                f"cert={float(cert_loss.item()):.4f} "
                f"field={float(field_loss.item()):.4f} "
                f"dball={float(decision_ball_loss.item()):.4f} "
                f"patch_reg={float(patch_reg_loss.item()):.4f} "
                f"clean_replay={float(clean_replay_loss.item()):.4f}",
                flush=True,
            )
    ...
    return RepairEpochResult(
        ...
        accuracy=total_correct / normalizer,
        num_samples=total_samples,
        robust_loss=robust_total / batch_normalizer if robust_loss_fn is not None else None,
        cert_loss=cert_total / batch_normalizer if cert_loss_fn is not None else None,
        field_loss=field_total / batch_normalizer if field_loss_fn is not None else None,
        decision_ball_loss=decision_ball_total / batch_normalizer if decision_ball_loss_fn is not None else None,
        patch_reg_loss=patch_reg_total / batch_normalizer if lambda_patch_l2 > 0.0 else None,
        clean_replay_loss=clean_replay_total / batch_normalizer if lambda_clean_replay > 0.0 else None,
        clean_route_hit=clean_route_hit_total / batch_normalizer if cert_loss_fn is not None else None,
        cert_margin=cert_margin_total / batch_normalizer if cert_loss_fn is not None else None,
...
def evaluate_repair_model_under_attack(
    model: nn.Module,
    base_model: nn.Module,
    loader: DataLoader,
    device: torch.device,
    ...
    """Evaluate repaired accuracy on adversarially perturbed bug inputs."""
    from src.training.losses.robust import generate_pgd_adversarial_examples
    ...
    for batch in loader:
        ...
        patched_logits = model(adv_inputs)
        predictions = torch.argmax(patched_logits, dim=1)  # [batch]
        total_correct += float((predictions == labels).float().sum().item())
        total_samples += int(labels.size(0))
